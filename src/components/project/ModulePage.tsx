...
function ConstructionAdvanced() {
  ...
  return (
    <section className="panel wide-panel">
      ...
      <div className="table-wrap xl">
        <table>
          <thead><tr>{["ماه", "تاریخ", "CAPEX تعدیل‌شده", "حقوق", "پیمانکار", "زیرساخت", "خروجی", "آورده", "وام", "کسری تأمین مالی", "نقد پایان", "وضعیت"].map((head) => <th key={head}>{head}</th>)}</tr></thead>
          <tbody>{outputs.construction.rows.map((row) => <tr key={row.monthNumber}><td>{formatNumber(row.monthNumber)}</td><td>{row.date}</td><td>{formatMoney(row.adjustedCapex, project)}</td><td>{formatMoney(row.developmentPayroll, project)}</td><td>{formatMoney(row.contractorCost, project)}</td><td>{formatMoney(row.infrastructureCost, project)}</td><td>{formatMoney(row.totalCashOutflow, project)}</td><td>{formatMoney(row.equityInjection, project)}</td><td>{formatMoney(row.debtDrawdown, project)}</td><td>{formatMoney(row.cashShortfall ?? 0, project)}</td><td>{formatMoney(row.endingCash, project)}</td><td className={row.cashCrunch ? "risk-cell" : "ok-cell"}>{row.cashCrunch ? "کسری" : "عادی"}</td></tr>)}</tbody>
        </table>
      </div>
    </section>
  );
