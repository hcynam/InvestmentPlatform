    ...
    <main className="management-cockpit">
      ...
      >
        ...
        <PremiumTableShell className="management-table-shell">
          <table>
            <thead><tr><th>ماه / تاریخ</th><th>مرحله مدل‌شده</th><th>CAPEX</th><th>CAPEX تجمعی</th><th>آورده</th><th>برداشت بدهی</th><th>خط اعتباری</th><th>خروج نقد</th><th>مانده نقد</th><th>وضعیت پوشش</th></tr></thead>
            <tbody>{view.constructionSeries.map((row) => <tr className={`funding-${row.fundingStatus}`} key={row.month}><td><strong>ماه {formatNumber(row.month)}</strong><small>{row.date}</small></td><td>{row.phase}</td><td>{row.capex === null ? "—" : formatMoney(row.capex, project)}</td><td>{row.cumulativeCapex === null ? "—" : formatMoney(row.cumulativeCapex, project)}</td><td>{row.equityDraw === null ? "—" : formatMoney(row.equityDraw, project)}</td><td>{row.debtDraw === null ? "—" : formatMoney(row.debtDraw, project)}</td><td>{row.creditLineDraw === null ? "—" : formatMoney(row.creditLineDraw, project)}</td><td>{row.totalOutflow === null ? "—" : formatMoney(row.totalOutflow, project)}</td><td>{row.endingCash === null ? "—" : formatMoney(row.endingCash, project)}</td><td><StatusPill tone={row.fundingStatus === "uncovered" ? "danger" : row.fundingStatus === "credit-line" ? "warning" : "success"}>{row.fundingStatus === "uncovered" ? "پوشش‌نیافته" : row.fundingStatus === "credit-line" ? "با خط اعتباری" : "کافی"}</StatusPill></td></tr>)}</tbody>
          </table>
        </PremiumTableShell>
      </DashboardSection>
